...
export class LastFeedComponent
  ...
  private listenFilters() {
    ...
    const subsDateRange = this.filterService.onDateRangeUpdate$.subscribe(
      (dateRange) => {
  ...
  setColumnDefs() {
    this.columnDefs = [
      ...
      {
        headerName:
          this.lastFeed.length > 0 ? this.lastFeed[0].field1_desc : "",
        field: "field1",
      },
      {
        headerName:
          this.lastFeed.length > 0 ? this.lastFeed[0].field2_desc : "",
        field: "field2",
      },
      {
        headerName:
          this.lastFeed.length > 0 ? this.lastFeed[0].field3_desc : "",
        field: "field3",
      },
      {
        headerName:
          this.lastFeed.length > 0 ? this.lastFeed[0].field4_desc : "",
        field: "field4",
      },
      {
        headerName:
          this.lastFeed.length > 0 ? this.lastFeed[0].field5_desc : "",
        field: "field5",
      },
      {
  ...
  private getProductionCount() {
    ...
    this.dashboardService
      .productionCount(dateIni, dateFin, this.channelId)
      .subscribe(
        (result) => {
          ...
          validResultSet.forEach((table) => {
            ...
            totalizer.mediaTaxa =
              Math.round(
                (totalizer.mediaTaxa /
                  (table.length >= 6 ? table.length - 1 : table.length)) *
                  100
              ) / 100;
  ...
  exportProductionExcel() {
    ...
    this.dashboardService
      .exportProductionExcel(dateIni, dateFin, this.channelId)
      .subscribe(
        (result) => {
          if (result.data.size > 0) {
            ...
            this.toastr.warning(
              "Não existe dados com os filtros selecionados.",
              "Aviso!",
              { enableHTML: true, showCloseButton: true }
            );
          }
        },
        (error) => {
  ...
  exportPauseExcel() {
    ...
    this.dashboardService
      .exportPauseExcel(dateIni, dateFin, this.channelId, this.machineCode)
      .subscribe(
        (result) => {
          if (result.data.size > 0) {
            let url = window.URL.createObjectURL(result.data);
